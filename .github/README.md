## Overview
This is a fork of OpenTripPlanner that replaces the default optimization criterion with one based on the number
of alternative paths available from each stop along a journey.

## Building this version
- bash get_hsl_files.sh (this is to get current hsl data, but will not rebuild the graph.obj)
...
- java -Xmx8G -jar otp-shaded/target/otp-shaded-2.10.0-SNAPSHOT.jar --build --save ./hsl (this is if the graph.obj needs to be updated)
